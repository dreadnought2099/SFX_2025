...
@section('main-content')
    <div class="p-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('products.create') }}" class text-white font-bold py-2 px-4 rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach ($products as $prod)
                <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $prod->name }}</h3>
                        <p class="text-gray-600 mb-4">{{ $prod->description }}</p>
                        <div class="flex justify-between items-center">
                            <span class="text-xl font-bold text-green-600">${{ number_format($prod->price, 2) }}</span>
                            <span
